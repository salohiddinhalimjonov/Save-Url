...
class UserRegisterForm(UserCreationForm):
    ...
    email = forms.EmailField(required=True,
                             widget=forms.EmailInput(attrs={'class': 'form-control', 'style': 'width:500px;'}))
...
class UrlForm(ModelForm):
    ...
    original_url = forms.URLField(label="Enter the Url", widget=forms.TextInput(
        attrs={'class': 'form-control', 'placeholder': 'example.com', 'style': 'width: 500px;'}))
    choices = forms.ChoiceField(choices=UrlChoices, label="Select a choice",
                                widget=forms.Select(attrs={'class': 'form-control', 'style': 'width: 500px;'}))
    description = forms.CharField(label="Enter the Description", widget=forms.Textarea(
        attrs={'rows': 10, 'class': 'form-control', 'placeholder': 'Enter the Description', 'style': 'width: 500px;'}))
    title = forms.CharField(label="Enter the Title", widget=forms.TextInput(
        attrs={'class': 'form-control', 'style': 'width: 500px;', 'placeholder': 'Title'}))
